...
const cores = process.env.NO_COLOR ? null : {
  reset: '\u001b[0m', forte: '\u001b[1m', fraco: '\u001b[90m',
  ok: '\u001b[32m', aviso: '\u001b[33m', erro: '\u001b[31m', frio: '\u001b[36m',
};
const pinta = (texto, cor) => (cores && cor ? cores[cor] + texto + cores.reset : texto);
...
function imprimir() {
  const larguraCategoria = Math.max(...linhas.map((l) => l.categoria.length), 10) + 2;

  console.log('');
  for (const linha of linhas) {
    const rotulo = linha.categoria.padEnd(larguraCategoria);
    const corpo = `${linha.total} declarados, ${linha.detalhe}`;
    console.log(`  ${pinta(rotulo, 'forte')}${linha.ruins ? pinta(corpo, 'aviso') : corpo}`);
  }

  if (problemas.length) {
    console.log('');
    console.log(pinta('  CORRIJA', 'erro') + pinta('  impede o item de funcionar', 'fraco'));
    for (const problema of problemas) {
      console.log(`    ${problema.onde}`);
      console.log(`      ${problema.oque}`);
      if (problema.comoResolver) console.log(pinta(`      > ${problema.comoResolver}`, 'frio'));
    }
  }

  if (suspeitas.length) {
    console.log('');
    console.log(pinta('  CONFIRME', 'aviso') + pinta('  sinalizado a partir do texto, pode ser falso positivo', 'fraco'));
    for (const suspeita of suspeitas) {
      console.log(`    ${suspeita.onde}: ${suspeita.oque}`);
    }
  }

  if (validar.length) {
    console.log('');
    console.log(pinta('  VALIDE A MAO', 'frio') + pinta(`  ${validar.length} hook(s) podem bloquear acoes`, 'fraco'));
    for (const item of validar) {
      const alvo = item.matcher ? `  ${item.matcher}` : '';
      console.log(`    ${item.nome.padEnd(34)} ${item.evento}${alvo}`);
    }
    console.log(pinta('    conseguem rodar, mas so voce sabe se ainda decidem o que deveriam', 'fraco'));
  }

  console.log('');
  console.log(pinta('  Nao verificado: bibliotecas importadas pelos scripts, arquivos de', 'fraco'));
  console.log(pinta('  configuracao que eles leiam, e a logica de qualquer hook.', 'fraco'));
  console.log('');
