...
function coletar(pacote) {
  ...
  if (!indice.length) {
    ui.item('settings.local.json de projeto', 'nenhum encontrado', 'neutro');
    return 0;
  }

  fs.mkdirSync(destino, { recursive: true });
  fs.writeFileSync(path.join(destino, INDICE), JSON.stringify(indice, null, 2), 'utf8');
  ui.item('settings.local.json de projeto', `${indice.length} projetos`);
  return 0;
}
...
function restaurar(pacote, simular) {
  ...
  try {
    ...
    ui.item('indice de projetos', 'ilegivel, pulando', 'aviso');
    return 0;
  }
  ...
  ui.item('settings.local.json', partes.join(', '));
  return 0;
}
